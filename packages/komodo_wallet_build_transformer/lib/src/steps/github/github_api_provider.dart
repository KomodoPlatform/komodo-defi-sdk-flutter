...
class GithubApiProvider {
  ...
  /// Creates a new instance of [GithubApiProvider]. The [githubToken] parameter
  /// is the GitHub API token used to authenticate requests. The [owner]
  /// parameter is the owner of the repository. The [repo] parameter is the name
  /// of the repository.
  ...
  Future<GitHubFile> getFileMetadata(String filePath) async {
    final String fileMetadataUrl = '$_baseUrl/contents/$filePath?ref=$_branch';
  ...
  Future<List<GitHubFile>> getDirectoryContents(
    ...
    _log.fine('Directory $repoPath contains ${data.length} items');
    _log.fine('\t of which ${files.length} are files');
