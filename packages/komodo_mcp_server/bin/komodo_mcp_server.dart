...
class KomodoMcpServer {
  ...
  // Tool implementations are the same as previously implemented _toolsCall switch
  Future<Map<String, Object?>> callTool(String name, Map<String, Object?> args) async {
    switch (name) {
      case 'sdk.call': {
        final method = args['method'] as String?;
        final methodParams = (args['params'] as Map<String, Object?>?) ?? <String, Object?>{};
        ...
        return {'content': [mcp.JsonContent(json: response)]};
      }
      case 'sdk.version': {
        final version = await _sdk.client.rpc.version();
        return {'content': [mcp.TextContent(text: version ?? 'unknown')]};
      }
      case 'auth.signIn': {
        final walletName = args['walletName'] as String?;
        final password = args['password'] as String?;
        if (walletName == null || password == null) throw ArgumentError('walletName/password required');
        final user = await _sdk.auth.signIn(walletName: walletName, password: password);
        return {'content': [mcp.JsonContent(json: user.toJson())]};
      }
      case 'auth.register': {
        final walletName = args['walletName'] as String?;
        final password = args['password'] as String?;
      ...
      case 'auth.currentUser': {
        ...
        return {'content': [user == null ? mcp.TextContent(text: 'null') : mcp.JsonContent(json: user.toJson())]};
      }
      case 'auth.signOut': {
        await _sdk.auth.signOut();
        return {'content': [mcp.TextContent(text: 'signed out')]};
      }
      case 'auth.users': {
        final users = await _sdk.auth.getUsers();
        return {'content': [mcp.JsonContent(json: users.map((u) => u.toJson()).toList())]};
      }
      case 'assets.listAvailable': {
        final assets = _sdk.assets.available.values.map((a) => a.toJson()).toList(growable: false);
        return {'content': [mcp.JsonContent(json: assets)]};
      }
      case 'assets.findByTicker': {
        ...
        return {'content': [mcp.JsonContent(json: matches)]};
      }
      case 'assets.activate': {
        ...
        return {'content': [mcp.TextContent(text: 'activated')]};
      }
      case 'assets.enabledTickers': {
        final tickers = await _sdk.assets.getEnabledCoins();
        return {'content': [mcp.JsonContent(json: tickers.toList())]};
      }
      case 'pubkeys.get': {
        final ticker = args['ticker'] as String?;
        if (ticker == null) throw ArgumentError('ticker required');
        ...
        return {'content': [mcp.JsonContent(json: pubkeys.toJson())]};
      }
      case 'pubkeys.new': {
        final ticker = args['ticker'] as String?;
        if (ticker == null) throw ArgumentError('ticker required');
        ...
        return {'content': [mcp.JsonContent(json: newKey.toJson())]};
      }
      case 'addresses.validate': {
        final ticker = args['ticker'] as String?;
        final address = args['address'] as String?;
        if (ticker == null || address == null) throw ArgumentError('ticker/address required');
        final candidates = _sdk.assets.findAssetsByConfigId(ticker).toList();
        if (candidates.isEmpty) throw ArgumentError('Unknown asset ticker');
        final validation = await _sdk.addresses.validateAddress(asset: candidates.first, address: address);
        ...
        return {'content': [mcp.JsonContent(json: validationJson)]};
      }
      case 'balances.get': {
        final ticker = args['ticker'] as String?;
        if (ticker == null) throw ArgumentError('ticker required');
        ...
        return {'content': [mcp.JsonContent(json: bal.toJson())]};
      }
      case 'tx.history': {
        final ticker = args['ticker'] as String?;
        ...
        return {'content': [mcp.JsonContent(json: jsonObj)]};
      }
      case 'signing.signMessage': {
        ...
        return {'content': [mcp.TextContent(text: sig)]};
      }
      case 'signing.verifyMessage': {
        ...
        final address = args['address'] as String?;
        if (coin == null || message == null || signature == null || address == null) {
        ...
        return {'content': [mcp.JsonContent(json: {'valid': ok})]};
      }
      case 'security.getPrivateKeys': {
        ...
        final mode = modeStr == null ? null : (modeStr.toLowerCase() == 'hd' ? KeyExportMode.hd : KeyExportMode.iguana);
        ...
        return {'content': [mcp.JsonContent(json: jsonMap)]};
      }
      case 'market.price': {
        final ticker = args['ticker'] as String?;
        ...
        return {'content': [mcp.JsonContent(json: {'price': price.toString()})]};
      }
      case 'fees.utxoEstimate': {
        final ticker = args['ticker'] as String?;
        if (ticker == null) throw ArgumentError('ticker required');
        final est = await _sdk.fees.getUtxoEstimatedFee(ticker);
        return {'content': [mcp.JsonContent(json: est.toJson())]};
      }
      ...
      case 'withdraw.preview': {
        ...
        final params = WithdrawParameters(asset: ticker, toAddress: toAddress, amount: Decimal.parse(amountStr));
        final preview = await _sdk.withdrawals.previewWithdrawal(params);
        return {'content': [mcp.JsonContent(json: preview.toJson())]};
      }
...
Future<int> _run(List<String> args) async {
  ...
  final mcpServer = mcp.McpServer(name: 'komodo-mcp', version: '0.1.0');

  // Register tools
  void registerTool(String name, String description, Map<String, Object?> schema) {
    mcpServer.registerTool(
      mcp.Tool(
        name: name,
        description: description,
        inputSchema: schema,
        handler: (args) async => (await server.callTool(name, Map<String, Object?>.from(args ?? {}))),
      ),
    );
  }
  ...
  registerTool('sdk.version', 'Get KDF version', {'type': 'object', 'properties': {}});
  ...
  registerTool('auth.register', 'Register', {
    'type': 'object',
    'required': ['walletName', 'password'],
    'properties': {'walletName': {'type': 'string'}, 'password': {'type': 'string'}, 'mnemonic': {'type': 'string'}},
  });
  registerTool('auth.currentUser', 'Get current user', {'type': 'object', 'properties': {}});
  registerTool('auth.signOut', 'Sign out', {'type': 'object', 'properties': {}});
  registerTool('auth.users', 'List users', {'type': 'object', 'properties': {}});

  registerTool('assets.listAvailable', 'List assets', {'type': 'object', 'properties': {}});
  ...
  registerTool('assets.enabledTickers', 'Enabled tickers', {'type': 'object', 'properties': {}});
  ...
  registerTool('tx.history', 'Get transaction history', {
    'type': 'object',
    'required': ['ticker'],
    'properties': {'ticker': {'type': 'string'}, 'pageNumber': {'type': 'integer'}, 'itemsPerPage': {'type': 'integer'}},
  });

  registerTool('signing.signMessage', 'Sign message', {
    'type': 'object',
    'required': ['coin', 'message', 'address'],
    'properties': {'coin': {'type': 'string'}, 'message': {'type': 'string'}, 'address': {'type': 'string'}},
  });
  registerTool('signing.verifyMessage', 'Verify message', {
    'type': 'object',
    'required': ['coin', 'message', 'signature', 'address'],
    'properties': {'coin': {'type': 'string'}, 'message': {'type': 'string'}, 'signature': {'type': 'string'}, 'address': {'type': 'string'}},
  });
  ...
  registerTool('market.price', 'Get fiat price', {
    'type': 'object',
    'required': ['ticker'],
    'properties': {'ticker': {'type': 'string'}, 'fiatCurrency': {'type': 'string'}, 'dateMs': {'type': 'integer'}},
  });
  ...
  registerTool('withdraw.preview', 'Preview withdrawal', {
    ...
    'properties': {'ticker': {'type': 'string'}, 'toAddress': {'type': 'string'}, 'amount': {'type': 'string'}},
  });
  registerTool('withdraw.execute', 'Execute withdrawal', {
    'type': 'object',
    'required': ['ticker', 'toAddress', 'amount'],
    'properties': {'ticker': {'type': 'string'}, 'toAddress': {'type': 'string'}, 'amount': {'type': 'string'}, 'feePriority': {'type': 'string', 'enum': ['low', 'medium', 'high']}},
  });

  // Start stdio transport
  final transport = mcp.StdioServerTransport(stdin: stdin, stdout: stdout);
  await mcpServer.serve(transport);
  return 0;
}
